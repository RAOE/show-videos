...
var selectBgm = function() {
	$('#allBgm').bootstrapTable({
		method : 'post',
		url : "../../adminUser/queryAllAdminUser",
		dataType : "json",
		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					console.log(row);
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.realName + '</span>';
					return id;
				}
			},
		
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.email + '</span>';
					return id;
				}
			},

			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.phoneNumber + '</span>';
					return id;
				}
			},{
				title : '操作',
